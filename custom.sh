...
./bin/solvetruss ${dirname}/frames gravity=${gravity} timef=${timef} srate=${srate} frate=${frate} fsize=${fwidth}x${fheight} "fcenter=(${fcenterx} ${fcentery})" fzoom=${fzoom} fscale=${fscale} < $filename > $dirname/solution.txt

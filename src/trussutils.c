	...
	else if(strcmp(argv[1], "mechanisms") == 0)
	{
		...
		for(int lv = 0; lv < lever_count; lv++)
		{
					 ...
					 &lever.jcount, &mindex, &lever.height, &lever.mass, &lever.stiffness, &lever.dampening) != 6)
			{
				...
				fprintf(stderr, "usage: lever line: joints=count member=[index] height=float mass=float stiffness=float dampening=float\n");
				return 1;
			}
			for(int j = 0; j < lever.jcount; j++)
			{
				int jindex;
				if(scanf("joint=[%d]\n", &jindex) != 1)
				{
				...
				if(jindex < 0 || jindex >= jcount)
				{
					fprintf(stderr, "error: index: lever [%d] line: joint [%d] parameter: [%d] does not exist\n", lv + 1, j + 1, jindex + 1);
					return 1;
				}
			}
			mindex--;
